        ...
        <div className="col-span-12 lg:col-span-8 flex flex-col gap-4">
            ...
            {slots.map((slot) => {
              ...
              return (
                <div key={slot.key} className={`rounded-2xl p-4 flex flex-col items-center justify-center text-center transition relative group ${
                  imgUrl ? 'border border-emerald-300 shadow-sm bg-white' : 'bg-slate-50/70 border-2 border-dashed border-slate-300 hover:border-red-300'
                }`}>
                  {imgUrl ? (
                  ...
                  <label className="cursor-pointer bg-[#a81c1c] hover:bg-[#8e1717] text-white font-bold text-xs px-4 py-2 rounded-xl shadow-sm transition">
                    Upload Image
          ...
          <div className="flex items-center justify-between gap-4 mt-2">
            ...
            <div className="flex items-center gap-3 shrink-0">
               <button onClick={() => { setImages({ front: '', back: '', side: '', labelCloseUp: ''}); setActivePreset(''); }} className="border border-slate-300 bg-white hover:bg-slate-50 text-slate-700 font-bold text-sm px-5 py-3 rounded-xl flex items-center gap-2 transition shadow-sm cursor-pointer">
                 <RefreshCw className="w-4 h-4" /> Reset
               </button>
               <button onClick={handleStartAnalysis} disabled={!hasMinimumImages || isAnalyzing} className={`font-extrabold text-sm px-6 py-3 rounded-xl flex items-center gap-2.5 transition cursor-pointer ${
