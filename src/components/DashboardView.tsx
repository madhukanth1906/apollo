        ...
        <div className="p-4 sm:p-5 lg:p-6 grid grid-cols-1 md:grid-cols-12 gap-4 lg:gap-6 items-center">
          ...
          <div className="md:col-span-6 lg:col-span-6 space-y-1.5 min-w-0">
            <div className="flex items-center gap-2">
              <span className="text-[10px] font-extrabold tracking-widest text-[#8b1515] uppercase bg-red-50 border border-red-200 px-2 py-0.5 rounded">
                {t('division_name', 'LEGAL METROLOGY DIVISION')}
              </span>
              <span className="text-[10px] text-slate-400 font-bold">•</span>
              <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">
                {t('portal_subtitle', 'FIELD INSPECTOR PORTAL')}
              </span>
            </div>

            <h1 className="text-xl sm:text-2xl lg:text-3xl font-black text-slate-900 tracking-tight leading-tight">
              {t('dashboard_welcome', 'Welcome back, Inspector!')}
            </h1>

            <p className="text-xs text-slate-600 font-medium leading-relaxed max-w-xl">
              {t('dashboard_sub', 'Scan, verify and ensure statutory compliance of packaged commodities under the Legal Metrology (Packaged Commodities) Rules, 2011.')}
            </p>
          </div>
          ...
          <div className="md:col-span-3 lg:col-span-3 border-y md:border-y-0 md:border-l border-slate-200 py-3 md:py-0 md:px-4 text-center md:text-left flex flex-col justify-center min-w-0">
            <span className="text-[9.5px] font-bold uppercase tracking-wider text-slate-400 mb-0.5">
              {t('motto_title', 'NATIONAL ENFORCEMENT MOTTO')}
            </span>
            <div className="font-serif italic font-bold text-base sm:text-lg text-[#8b1515] leading-snug">
              {t('motto_text', '"Fair Measurement Builds a Fair Nation"')}
            </div>
            <span className="text-[9px] text-slate-500 mt-1 font-sans">
              {t('motto_dept', 'Department of Consumer Affairs • Govt. of India')}
            </span>
          </div>
            ...
            <div className="bg-slate-50 border border-slate-200 rounded-xl p-3 flex items-center gap-3 shadow-2xs w-full sm:w-auto min-w-[190px]">
              ...
              <div className="min-w-0 leading-tight">
                ...
                <div className="text-[9.5px] text-emerald-700 font-semibold flex items-center gap-1 mt-0.5">
                  ...
                  <span>{t('live_sync', 'On Duty • Live Sync')}</span>
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3.5 sm:gap-4">
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-4 flex items-center justify-between shadow-2xs">
          <div className="min-w-0">
            <p className="text-[10px] text-slate-500 font-bold uppercase tracking-wider mb-1">{t('stat_total_inspections', 'TOTAL INSPECTIONS')}</p>
            ...
            <p className="text-[10px] text-slate-400 mt-1">{t('stat_total_sub', 'National Registry Count')}</p>
          </div>
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-4 flex items-center justify-between shadow-2xs">
          <div className="min-w-0">
            <p className="text-[10px] text-slate-500 font-bold uppercase tracking-wider mb-1">{t('stat_compliant', 'COMPLIANT PACKAGES')}</p>
            <div className="flex items-baseline gap-2">
            ...
            <p className="text-[10px] text-slate-400 mt-1">{t('stat_compliant_sub', 'PCR 2011 Certified')}</p>
          </div>
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-4 flex items-center justify-between shadow-2xs">
          <div className="min-w-0">
            <p className="text-[10px] text-slate-500 font-bold uppercase tracking-wider mb-1">{t('stat_violations', 'NON-COMPLIANT / SEIZURE')}</p>
            <div className="flex items-baseline gap-2">
              <h4 className="text-2xl font-black text-[#8b1515] leading-none">52</h4>
              <span className="text-[11px] font-bold text-red-600">21.0%</span>
            </div>
            <p className="text-[10px] text-slate-400 mt-1">{t('stat_violations_sub', 'Notices Issued Sec 36(1)')}</p>
          </div>
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-4 flex items-center justify-between shadow-2xs">
          <div className="min-w-0">
            <p className="text-[10px] text-slate-500 font-bold uppercase tracking-wider mb-1">{t('stat_confidence', 'METROLOGY CONFIDENCE')}</p>
            <div className="flex items-baseline gap-2">
            ...
            <p className="text-[10px] text-slate-400 mt-1">{t('stat_confidence_sub', 'OCR & ArUco Homography')}</p>
          </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3.5 sm:gap-4">
        ...
        >
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <h3 className="font-bold text-slate-900 text-xs truncate">{t('step_1_title', '1. Scan Package')}</h3>
              <p className="text-[10px] text-slate-500 font-medium truncate">{t('step_1_desc', 'Multi-view PDP capture')}</p>
            </div>
          </div>
        ...
        >
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <h3 className="font-bold text-slate-900 text-xs truncate">{t('step_2_title', '2. AI Extraction')}</h3>
              <p className="text-[10px] text-slate-500 font-medium truncate">{t('step_2_desc', '7 mandatory declarations')}</p>
            </div>
          </div>
        ...
        >
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <h3 className="font-bold text-slate-900 text-xs truncate">{t('step_3_title', '3. Rule Audit')}</h3>
              <p className="text-[10px] text-slate-500 font-medium truncate">{t('step_3_desc', 'Check PCR 2011 clauses')}</p>
            </div>
          </div>
        ...
        >
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <h3 className="font-bold text-slate-900 text-xs truncate">{t('step_4_title', '4. Official Dossier')}</h3>
              <p className="text-[10px] text-slate-500 font-medium truncate">{t('step_4_desc', 'Form LM-1 PDF export')}</p>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 items-start">
        ...
        <div className="lg:col-span-8 bg-white border border-slate-200 rounded-xl p-5 shadow-xs flex flex-col gap-4">
          <div className="flex items-center gap-2.5">
            ...
            <div className="min-w-0">
              <h2 className="text-base font-bold text-slate-900 leading-tight">
                {t('upload_title', 'Upload Commodity Package Images')}
              </h2>
              <p className="text-xs text-slate-500 mt-0.5">
                {t('upload_sub', 'Upload multi-angle high-resolution photographs (Front PDP, Back Legal, Side, Macro Close-up) for statutory verification.')}
              </p>
            </div>
          </div>
          ...
          <div 
            ...
            onDrop={handleDrop}
            className="bg-slate-50/60 border-2 border-dashed border-slate-300 hover:border-[#8b1515] rounded-xl p-6 sm:p-8 flex flex-col items-center justify-center text-center gap-2.5 transition min-h-[200px]"
          >
            <UploadCloud className="w-12 h-12 text-slate-400" />
            <p className="text-sm font-bold text-slate-800">
              {t('drag_drop_text', 'Drag & drop package photographs here')}
            </p>
            <p className="text-xs text-slate-400 font-medium">{t('or', 'or')}</p>
            <div className="flex flex-wrap items-center justify-center gap-3">
              <button 
                type="button"
                onClick={(e) => { e.stopPropagation(); handleOpenCamera(); }}
                className="bg-slate-800 hover:bg-slate-900 text-white font-bold text-xs sm:text-sm px-5 py-2.5 rounded-lg shadow-xs flex items-center gap-2 transition cursor-pointer"
              >
                <Camera className="w-4 h-4" /> 
                <span>{t('use_camera', 'Use Camera')}</span>
              </button>
              <button 
                type="button"
                onClick={(e) => { e.stopPropagation(); handleBrowseFiles(); }}
                className="bg-[#8b1515] hover:bg-[#780e0e] text-white font-bold text-xs sm:text-sm px-5 py-2.5 rounded-lg shadow-xs flex items-center gap-2 transition cursor-pointer"
              >
                <FolderOpen className="w-4 h-4" /> 
                <span>{t('browse_files', 'Browse Local Files')}</span>
              </button>
            </div>
            <p className="text-[11px] text-slate-400 font-medium mt-1">
              {t('supports_types', 'Supports: JPEG, PNG, WEBP • Max 15 MB per evidence photograph')}
            </p>
          ...
          <div className="bg-amber-50/70 border border-amber-200 rounded-lg p-3 flex items-start gap-2 text-xs text-amber-900">
            ...
            <div>
              <strong>{t('inspector_tip_label', 'Inspector Tip:')}</strong> {t('inspector_tip_text', 'Ensure adequate illumination and minimize surface glare. For Rule 9 numeral font verification, place the certified ArUco reference marker in the same plane.')}
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-4 bg-white border border-slate-200 rounded-xl p-5 shadow-xs flex flex-col gap-3.5">
          <div className="flex items-center justify-between border-b border-slate-100 pb-3">
            <h2 className="text-sm font-bold text-slate-900 uppercase tracking-wide">
              {t('recent_ledger', 'Recent Inspection Ledger')}
            </h2>
            <button 
              onClick={onOpenHistory} 
              className="text-xs font-bold text-[#8b1515] hover:underline flex items-center gap-1 cursor-pointer"
            >
